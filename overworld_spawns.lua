...
return function(mod)
  ...
  do
    local dirPath = mod.path .. "/" .. WALKERS_REL
    local ok, files = pcall(love.filesystem.getDirectoryItems, dirPath)
    if ok and files then
      ...
      mod.log:warn("galar_gmax_dex: could not list %s for wild sprite registration", dirPath)
    end
  end
  ...
  -- already used for battle sprites elsewhere in this mod. Was briefly a
  -- live Mod Manager option (wild_display_height); explicit user decision
  -- to settle it as a fixed constant instead and drop it from options.lua.
